  ...
  return (
    ...
    >
      ...
      <div className="relative z-20 min-h-[calc(100vh-4rem)] flex flex-col items-center justify-center text-center max-w-5xl mx-auto w-full px-4">
        ...
        {/* The Exact Skiper19 SVG Scribble & Trail for Desktop, Tablet & TV (100% untouched) */}
        <LinePath
          className="hidden md:block absolute top-1/2 -translate-y-[240px] left-1/2 -translate-x-1/2 -z-10 pointer-events-none w-[1278px] h-auto overflow-visible opacity-85"
...
function generateMobilePath(basePath: string): { path: string; startX: number; startY: number } {
  const commands = basePath.match(/([MC])([^MC]+)/g) || []
  const result: string[] = []

  // Center scribble in mobile 390px coordinate space
  ...
  const baseSpanX = 1327.14
  const targetMinX = 35
  const targetSpanX = 320
  ...
  commands.forEach((cmd, idx) => {
    ...
    if (idx <= 35) {
      // Scribble cluster: centered around X = 195, height fitted to hero
      const transformed: number[] = []
      for (let i = 0; i < nums.length; i += 2) {
        const x = scribbleCenterX + (nums[i] - baseScribbleCenterX) * scribbleK
        const y = nums[i + 1] * scribbleK + scribbleYOffset
        transformed.push(Number(x.toFixed(2)), Number(y.toFixed(2)))
        lastX = x
        lastY = y
      }
      result.push(`${type}${transformed.join(' ')}`)
    } else {
      // Trail: weaving naturally across the mobile screen bounds
      const transformed: number[] = []
      for (let i = 0; i < nums.length; i += 2) {
        const x = targetMinX + (nums[i] - baseMinX) * (targetSpanX / baseSpanX)
        const y = trailStartY + (nums[i + 1] - baseStartY) * (targetSpanY / baseSpanY)
        transformed.push(Number(x.toFixed(2)), Number(y.toFixed(2)))
        lastX = x
        lastY = y
      }
  ...
  return {
    path: result.join(''),
    startX: Number(lastX.toFixed(2)),
    startY: Number(lastY.toFixed(2)),
  }
}

const MOBILE_DATA = generateMobilePath(BASE_PATH)
...
const MobileLinePath = ({
  ...
  useEffect(() => {
    const updatePath = () => {
      ...
      const scale = 390 / svgRect.width
      const distanceToBottom = containerRect.bottom - svgRect.top
      const startX = MOBILE_DATA.startX
      const startY = MOBILE_DATA.startY
      ...
      const slope = 0.15
      const endX = Math.min(350, Math.max(40, startX + slope * Math.min(deltaY, 180)))
      const endY = targetY
  ...
  return (
    ...
    >
      ...
      <path
        ...
        strokeLinecap="round"
      />
      {/* Active Animated CodeBits Emerald Path on Mobile */}
      <motion.path
        d={fullPath}
        stroke="#00C269"
        strokeWidth="7"
